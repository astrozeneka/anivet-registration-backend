...
class ScientistDAO extends BaseCrudDAO {
    ...
    async buildTable(){
        ...
        await sqlExecute("" +
            "CREATE VIEW `scientist_edit` AS" +
            "   SELECT * FROM scientist") // นักวีทไม่มีะที่อยู่
        await sqlExecute("" +
            "CREATE VIEW `scientist_$search` AS " +
            "   SELECT *, CONCAT(baseMember_username,' ',baseMember_email,' ',baseMember_name1,' ',baseMember_name2,' ',baseMember_phone) AS s FROM `scientist`"
        )
    }
    ...
    async add(m){
        let d = await sqlExecute(
            ...
            "baseMember_name1, baseMember_name2, baseMember_phone, baseMember_email, baseMember_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
                ...
                m.name1, m.name2, m.phone, m.email, 'scientist'
            ]
        )
        m.id = d.insertId
        return m
